...
console.log(getOdds([1,2,9,6,3,,4,8]))
/**
 * getEvens(numbers):
 * - receives an array of numbers called `numbers`
 * - filters the `numbers` array in order to...
...
function getEvens(numbers) {
  // Your code here
  let z = numbers.filter(function (num) {
    return num %2 === 0;
  })
  return z;
}

console.log(getEvens([1,2,9,6,3,,4,8]))
/**
 ...
 */
function countOccurences(p, numbers) {
  let h = numbers.filter (function (number){
    return number === p 
  })
  return h.length 
}
...
console.log (makeThemDoctors(['basel','ahmad','khaled']))
